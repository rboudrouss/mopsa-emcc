...
function isMultilanguage(name: string) {
  return (
    name.toLowerCase().includes("multilanguage") ||
    name.toLowerCase().includes("multilangage")
  );
}
...
export function DomainsPanel() {
  ...
  const activeExt = activeFilePath
    ? (activeFilePath.split(".").pop() ?? "")
    : "";
  const activeLang: SupportedLanguage | null =
    SUPPORTED_EXTENSIONS[activeExt] ?? null;
  ...
  const handlePresetChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const [presetLang, presetName] = val.split("|") as [
      SupportedLanguage,
      string,
    ];
    const langConfigs =
      presets?.configs[presetLang as "c" | "python" | "universal" | "cfg"] ??
      {};
  ...
  const renderOptions = () => {
    ...
    const entries = Object.keys(configs).filter(
      (name) => !isMultilanguage(name),
    );
    ...
    <div style={{ display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "0 16px 10px", position: "relative" }}>
        {!isMultilang && activeLang === null ? (
          ...
          >
            Select a file with a supported extension (.c, .h, .py, .u) to
            configure domains.
          </div>
        ) : (
          <>
