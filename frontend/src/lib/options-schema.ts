    ...
    options: [
      {
        ...
        choices: ['all', 'range', 'callstack', 'range_callstack', 'range_control'],
      },
      {
        flag: '-stub-alloc-pol',
        type: 'select',
...
export const BOOL_ARG_FLAGS: Set<string> = new Set(
  OPTIONS_SCHEMA.flatMap((g) => g.options.filter((o) => o.type === 'boolArg').map((o) => o.flag))
);
...
export const MOPSA_DEFAULT_VALUES: Record<string, unknown> = Object.fromEntries(
  OPTIONS_SCHEMA.flatMap((g) =>
    g.options.map((o) => [o.flag, 'mopsaDefault' in o ? o.mopsaDefault : o.default])
  )
);
